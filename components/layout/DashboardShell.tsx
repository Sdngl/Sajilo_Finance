...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#f8faf9]">
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-[250px] bg-[#102a27] text-white transition-transform lg:translate-x-0 ${
        ...
        <div className="px-4">
          ...
          {items.map(([label, href, Icon]) => (
            <Link
              ...
              className={`mb-1 flex items-center gap-3 rounded-xl px-3 py-3 text-[13px] font-medium ${
                path === href || (href !== '/dashboard' && path.startsWith(href))
                  ? 'bg-[#1b574d] text-white'
                  : 'text-[#aec0bb] hover:bg-[#163d38] hover:text-white'
              }`}
            >
              <Icon size={17} />
              {label}
      ...
      <main className="lg:pl-[250px]">
        <header className="sticky top-0 z-30 flex h-20 items-center justify-between border-b border-[#e7eeeb] bg-[#f8faf9]/90 px-5 backdrop-blur md:px-10">
          ...
          <div className="relative hidden w-72 md:block">
            <Search className="absolute left-3 top-2.5 text-[#9aa9a4]" size={17} />
            <input
            ...
            <div className="relative">
              ...
              {profile && (
                <div className="absolute right-0 top-12 w-44 rounded-xl border border-[#e7eeeb] bg-white p-2 shadow-xl">
                  <p className="px-3 py-2 text-xs font-semibold">Navin Shrestha</p>
